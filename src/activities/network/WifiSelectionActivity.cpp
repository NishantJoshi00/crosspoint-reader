...
void WifiSelectionActivity::checkConnectionStatus() {
  ...
  if (status == WL_CONNECTED) {
    ...
    // Recover a clock that lost its date even if it synced before. Only use
    // this connection; sleeping and offline reading never start Wi-Fi for time.
    if (halClock.isAvailable() && (!SETTINGS.clockHasBeenSynced || !halClock.hasValidTime())) {
